...
def load_tiled_test_images(dir):
    ...
    for filename in os.listdir(dir):
        if filename.endswith('.jpg') or filename.endswith('.png'):
            ...
            if full_image_name not in images:
...
def compute_metrics(gt, pred_boxes, pred_conf, og_image, conf_thresh=0.6, iou_thresh=0.5, plot=False):
    ...
    if plot:
        print(
            f'Count Mae: {count_mae}, F1: {f1:.2f}, Precision: {precision:.2f}, \
...
if __name__ == '__main__':
    ...
    for image in tqdm(test_images, position=0, leave=True):
        ...
        with open(os.path.join(og_labels_path)) as f:
        ...
        if iou_sweep:
            ...
            for iou_thresh in iou_thresh_vals:
                count_mae, pr, re, f1 = compute_metrics(
                    instances_float, filtered_boxes, filtered_conf, og_image, iou_thresh=iou_thresh, plot=args.plot
                )
            ...
            count_mae, pr, re, f1 = compute_metrics(
                instances_float, filtered_boxes, filtered_conf, og_image, plot=args.plot
            )
            full_count_mae.append(count_mae)
            full_precision.append(pr)
            full_recall.append(re)
